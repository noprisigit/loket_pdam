...
class _TagihanState extends State<Tagihan> {
  ...
  Widget build(BuildContext context) {
    ...
    final formTagihan = Padding(
      padding: EdgeInsets.all(10.0),
        ...
        decoration: InputDecoration(
          ...
          focusedBorder: OutlineInputBorder(borderSide: BorderSide(color: HexColor('#1ab394')))
        ),
      )
    );
    ...
    final listPelangganNotFound = ListView(
      children: <Widget>[
    ...
    final listPelanggan = _issetData == false ? Text("") : Container(
      child: Column(
        ...
        children: <Widget>[
          Padding(
            padding: EdgeInsets.all(10.0),
            child: Text("Nama Pelanggan : ${listData[0]['nama_pelanggan']}")
          ),
          Padding(
                      ...
                      children: <Widget>[
                        Row(
                          children: <Widget>[
                            Text("Bulan / Tahun"),
                            Text(" : "),
                            Text("${listData[index]['bulan']} / ${listData[index]['tahun']}")
                          ],
                        ),
                        Row(
                          children: <Widget>[
                        ...
                        Row(
                          children: <Widget>[
                            Text("Total Tagihan "),
                            Text(" : "),
                            Text("${listData[index]['total_tagihan']}")
                          ],
                        ),
                        // Text("Bulan              : ${listData[index]['bulan']}"),
                        // Text("Tahun             : ${listData[index]['tahun']}"),
                        // Text("Tarif Air          : ${listData[index]['tarif_air']}"),
                        // Text("Tarif Non Air   : ${listData[index]['tarif_air']}"),
                        // Text("Denda              : ${listData[index]['tarif_air']}"),
                        // Text("Total Tagihan : ${listData[index]['tarif_air']}"),
                      ],
                    )
                  ),
                );
